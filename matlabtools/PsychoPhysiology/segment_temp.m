function out = segment_temp(cfg, data)
%function out = segment_temp(cfg, data)
% function to resegment the temp data from the Empatica Devices
%
...
if (pretrigger < 0)
    error(strcat('trigger_time of participant starts "',num2str(pretrigger),'" seconds before the temp data file'))
end
if ((pretrigger+cfg.posttrigger) > data.time(length(data.time)))
    error(strcat('posttrigger of participant end "',num2str((pretrigger+cfg.posttrigger)-data.time(length(data.time))),'" seconds after the temp data file'))
end
...
temperature = data.temperature(startpoint:endpoint,:);

%create a new linear time array from 0-posttrigger time with the amount of
%datapoints available in the acceleration list
time = rot90(flip(linspace(0,cfg.posttrigger+cfg.pretrigger,length(temperature))));
