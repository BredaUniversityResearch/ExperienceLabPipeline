function out = segment_hr(cfg, data)
% function out = segment_hr(cfg, data)
% function to resegment the hr data from the Empatica Devices
%
...
if (pretrigger < 0)
    error(strcat('trigger_time of participant starts "',num2str(pretrigger),'" seconds before the hr data file'))
end
if ((pretrigger+cfg.posttrigger) > data.time(length(data.time)))
    error(strcat('posttrigger of participant end "',num2str((pretrigger+cfg.posttrigger)-data.time(length(data.time))),'" seconds after the hr data file'))
end
...
heartrate = data.heartrate(startpoint:endpoint,:);

%create a new linear time array from 0-posttrigger time with the amount of
%datapoints available in the acceleration list
time = rot90(flip(linspace(0,cfg.posttrigger+cfg.pretrigger,length(heartrate))));
