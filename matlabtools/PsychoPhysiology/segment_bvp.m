function out = segment_bvp(cfg, data)
%function out = segment_bvp(cfg, data)
% function to resegment the bvp data from the Empatica Devices
%
...
if (pretrigger < 0)
    error(strcat('trigger_time of participant starts "',num2str(pretrigger),'" seconds before the bvp data file'))
end
if ((pretrigger+cfg.posttrigger) > data.time(length(data.time)))
    error(strcat('posttrigger of participant end "',num2str((pretrigger+cfg.posttrigger)-data.time(length(data.time))),'" seconds after the bvp data file'))
end
