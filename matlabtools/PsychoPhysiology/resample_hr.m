function out = resample_hr(cfg, data)
%function out = resample_hr(cfg, data)
% function to resample the heartrate data from the Empatica Devices (up or down)
%
...
if ~isfield (cfg,'fsample')
    error('no sample frequency is defined for resample_hr');
end
...
heartrate = resample(data.heartrate,data.time,cfg.fsample,1,1);

%calculate the total duration of the data, then generate an array of time
%points
tFinal = data.time(length(data.time));
time = rot90(flip(linspace(0,tFinal,length(heartrate))));
