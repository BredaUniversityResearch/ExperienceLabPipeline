...
for samp_i = 1:max(size(data))
    cfg.multipleparticipants = 0; % process one participant at a time first
...
function out = gridify_table(cfg,data)
    ...
    [data_g,~,idx] = unique(data(:,width(data)-2:width(data)),'rows'); % TODO: this works only if x, y, z are added in the previous step and are the last three columns
